...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().cors().and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS)
				...
				.authorizeRequests()
				.antMatchers(HttpMethod.POST, "/**/login", "/authen/register", "/applicant/api/applications").permitAll()
				.antMatchers(HttpMethod.GET, "/applicant/api/applications").hasAnyRole("STAFF, ADMIN")
				.antMatchers(HttpMethod.GET, "/logger/api/logs").hasAnyAuthority("logManagement")
